package com.desgreen.education.siapp.ui.util;

import com.desgreen.education.siapp.backend.model_sample.Address;
...
import com.desgreen.education.siapp.ui.util.css.*;
...
public class UIUtils {
	...
	private static final ThreadLocal<DateTimeFormatter> dateFormat_ddMMMYYYY = ThreadLocal
	...
	public static Label createBoldLabel(String strLabel) {
		Label label = createH5Label(strLabel);
		label.addClassName(LumoStyles.FontFamily.MONOSPACE);
		return label;
	}
	...
	public static String formatDate_ddMMMYYYY(LocalDate date) {
		return dateFormat_ddMMMYYYY.get().format(date);
	}
	...
	/**
	 * =========== TAMBAHAN SENDIRI DARI MAS BAGUS WINARNO ========
	 */
	public static Component createComponentFromFile(String mimeType, String fileName,
                                                    InputStream stream) {
		Image imageOuput = new Image();
		imageOuput.setTitle(fileName);
		if (mimeType.startsWith("text")) {
			return createTextComponentFromFile(stream);
		} else if (mimeType.startsWith("image")) {
			try {

				byte[] bytes = IOUtils.toByteArray(stream);
				imageOuput.getElement().setAttribute("src", new StreamResource(
				...
				try (ImageInputStream in = ImageIO.createImageInputStream(
					...
					if (readers.hasNext()) {
						...
						try {
							reader.setInput(in);
							imageOuput.setWidth(reader.getWidth(0) + "px");
							imageOuput.setHeight(reader.getHeight(0) + "px");
						} finally {
							reader.dispose();
						}
					}
	...
	public static Component createTextComponentFromFile(InputStream stream) {
		String text;
		try {
